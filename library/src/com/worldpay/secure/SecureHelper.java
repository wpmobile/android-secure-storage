...
public final class SecureHelper {
	 ...
	 * @return None.
	 */
	void generateKeyStore(String algorithm, String aliasName, String name, char[] password) {
	 ...
	 * @return key.
	 */
	Key getKey(String alias, char[] password, String name) {
	 ...
	 */
	boolean isKeyStoreExist(String filename) {
		FileInputStream fileInputStream = null;
	 ...
	 */
	String generatePassword(String randomNumber) {
	...
	boolean isIvEmpty() {
		...
		if (f.exists()) {
			...
			// Check the first IV entry. If it exists, we can assume they all
			// do.
			String iv = getIV(0);
			isEmpty = TextUtils.isEmpty(iv);
	...
	void clearAll(char[] password) {
		clearPreferences(FILE_NAME_IV);
		clearPreferences(FILE_NAME_RANDOM_NO);
		...
		context.deleteFile(SecureStorage.KEYSTORE_FILENAME);
